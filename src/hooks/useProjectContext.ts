...
export const useProjectContext = (directoryHandle: FileSystemDirectoryHandle | null) => {
    ...
    // Guard to prevent concurrent/duplicate scans firing multiple API storm requests
        ...
        try {
            ...
            const runRes = await fetch('/api/run-project', {
                ...
                body: JSON.stringify({
                    ...
                    isExternalProject: true,
                    projectContext: projectContext ? projectContext.substring(0, 500) : null
                })
            });
            if (!runRes.ok) {
    ...
    const scanProject = useCallback(async () => {
        ...
        // Prevent duplicate concurrent scans (e.g., double React StrictMode mount)
        if (scanActiveRef.current) {
            console.log("📂 scanProject: Scan already in progress, skipping duplicate.");
            return;
        }
        ...
        try {
            ...
            // Small delay to let the backend finish binding on (re)start before firing API calls
